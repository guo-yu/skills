...
share-skill 在创建或更新仓库时，会自动生成/更新 README.md 文件。

### 获取 GitHub 用户真实姓名

从配置的 GitHub 用户别名获取真实姓名：

```bash
# 从配置读取 GitHub 用户名
GITHUB_USERNAME=$(cat ~/.claude/share-skill-config.json | jq -r '.remotes.github' | grep -oP 'github\.com[:/]\K[^/]+')

# 访问 GitHub API 获取真实姓名
REAL_NAME=$(curl -s "https://api.github.com/users/$GITHUB_USERNAME" | jq -r '.name // empty')
...
| 仓库类型 | 标题格式 | 示例 |
|---------|---------|------|
| **Skill Set (公共仓库)** | `{真实姓名}'s Skills` | `Guo Yu's Skills` |
| **单个 Skill** | `{真实姓名}'s Skill: {skill-name}` | `Guo Yu's Skill: port-allocator` |

**判断逻辑：**
```bash
# 检查是否为 skill set 仓库（包含多个 skill 子目录）
SKILL_COUNT=$(find ~/Codes/skills -maxdepth 1 -type d -name "*" ! -name "docs" ! -name ".git" ! -name "skills" | wc -l)

if [ "$SKILL_COUNT" -gt 1 ]; then
  # Skill Set
  TITLE="${REAL_NAME}'s Skills"
else
  # 单个 Skill
  SKILL_NAME=$(basename "$SKILL_DIR")
  TITLE="${REAL_NAME}'s Skill: $SKILL_NAME"
fi
```
...
{skill 简介 - 从 SKILL.md 的 description 提取}

## Skills

| Skill | Description |
|-------|-------------|
| [port-allocator](./port-allocator/) | 自动分配端口... |
| [share-skill](./share-skill/) | 分享 skill 到仓库... |
...
### 单个 Skill 的 README 模板

```markdown
# {真实姓名}'s Skill: {skill-name}
...
This skill has an online documentation site.

**访问文档：**
- 自定义域名: https://{custom_domain}/?skill={skill-name}
- GitHub Pages: https://{username}.github.io/{repo-name}/docs/?skill={skill-name}
...
1. **获取 GitHub 用户信息**
   ...
   # 获取真实姓名（带超时和错误处理）
   REAL_NAME=$(curl -s --max-time 5 "https://api.github.com/users/$GITHUB_USERNAME" | jq -r '.name // empty')
...
4. **生成文档访问链接**
   ...
   if [ -n "$CUSTOM_DOMAIN" ]; then
     DOC_URL="https://$CUSTOM_DOMAIN/"
   else
     DOC_URL="https://${GITHUB_USERNAME}.github.io/${REPO_NAME}/docs/"
   fi
   ```

5. **写入 README.md**（追加模式，不覆盖用户自定义内容）
   ```bash
   # 检查是否存在自定义内容标记
   if grep -q "<!-- CUSTOM CONTENT START -->" README.md 2>/dev/null; then
     # 保留用户自定义部分，只更新自动生成部分
     # ...
   else
     # 全新生成
